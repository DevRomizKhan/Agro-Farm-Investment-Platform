...
function formatPostDate(post: BlogPostWithAuthor, lang: string) {
    ...
    <section id="blog" className="border-t border-white/5 bg-slate-900/40 py-24">
      <div className="section-container">
        <div className="mb-14 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-4 py-2 text-sm font-bold uppercase tracking-widest text-emerald-400">
              <BookOpen className="h-4 w-4" />
              {t.blog.badge}
            </div>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black leading-tight text-white">
              {t.blog.titlePrefix}<span className="gradient-text">{t.blog.titleHighlight}</span>
            </h2>
          </div>
          <Link
            href="/blog"
            className="group inline-flex shrink-0 items-center gap-2 text-base font-semibold text-emerald-400 transition-colors hover:text-emerald-300"
          >
            {t.blog.allArticles}
        ...
        {!featuredPost ? (
          <div className="rounded-3xl border border-dashed border-white/10 bg-slate-900/50 px-6 py-16 text-center">
            ...
            <h3 className="mb-2 text-2xl font-bold text-white">{t.blog.newInsightsTitle}</h3>
            <p className="mx-auto max-w-md text-base leading-relaxed text-slate-400">
              {t.blog.newInsightsDesc}
            </p>
          </div>
        ) : (
          <>
            <article className="grid items-stretch gap-8 lg:grid-cols-12 lg:gap-12">
              <div className="flex flex-col justify-between space-y-8 rounded-3xl border border-white/10 bg-slate-900/60 p-8 lg:col-span-6">
                <div>
                  <div className="mb-5 flex flex-wrap items-center gap-3">
                    {featuredPost.category && (
                      <span className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3.5 py-1.5 text-xs font-bold uppercase tracking-wider text-emerald-400">
                        {featuredPost.category}
                      </span>
                    )}
                    <span className="flex items-center gap-1.5 text-sm text-slate-400">
                      <Calendar className="h-4 w-4" />
                      {formatPostDate(featuredPost, lang)}
                    </span>
                  </div>
                  <h3 className="mb-4 text-3xl sm:text-4xl font-bold leading-snug text-white">
                    {featuredPost.title}
                  </h3>
                  {featuredPost.excerpt && (
                    <p className="line-clamp-4 text-base leading-relaxed text-slate-300 sm:text-lg">
                      {featuredPost.excerpt}
                    </p>
                  )}
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 border-t border-white/10 pt-5">
                  ...
                  <Link
                    ...
                    className="btn-primary h-11 px-5 text-sm group"
                  >
                    {t.blog.readArticle}
              ...
              >
                ...
                <div className="absolute bottom-5 left-5 right-5 rounded-2xl border border-white/10 bg-slate-950/75 px-5 py-3.5 backdrop-blur-md">
                  <p className="mb-0.5 text-xs font-bold uppercase tracking-wider text-emerald-400">{t.blog.featuredInsight}</p>
                  <p className="line-clamp-1 text-sm font-medium text-slate-200">{featuredPost.title}</p>
                </div>
              </Link>
            </article>
                    ...
                    <div className="p-5">
                      <div className="mb-2 flex items-center justify-between gap-3">
                        <span className="truncate text-xs font-bold uppercase tracking-wider text-emerald-400">
                          {post.category || t.blog.categoryDefault}
                        </span>
                        <span className="shrink-0 text-xs text-slate-400">{formatPostDate(post, lang)}</span>
                      </div>
                      <h4 className="line-clamp-2 text-base font-semibold leading-snug text-white">{post.title}</h4>
                    </div>
                  </Link>
                ))}
              </div>
