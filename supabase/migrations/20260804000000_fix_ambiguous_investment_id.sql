-- Qualify the transaction column because the function also returns an
-- investment_id field. This prevents PostgreSQL's ambiguous-column error.
...
BEGIN
  ...
  WHERE public.investments.id = p_investment_id
  FOR UPDATE;
  ...
  WHERE public.investment_plans.id = v_investment.plan_id
  FOR UPDATE;
  ...
  SELECT COALESCE(SUM(GREATEST(i.shares_purchased, 0)), 0)::INTEGER INTO v_active_shares
  FROM public.investments AS i
  WHERE i.plan_id = v_investment.plan_id AND i.status = 'active';

  SELECT COALESCE(SUM(GREATEST(i.shares_purchased, 0)), 0)::INTEGER INTO v_older_pending_shares
  FROM public.investments AS i
  WHERE i.plan_id = v_investment.plan_id
    AND i.status = 'pending'
    AND (i.created_at < v_investment.created_at OR (i.created_at = v_investment.created_at AND i.id < v_investment.id));

  SELECT COALESCE(SUM(GREATEST(i.shares_purchased, 0)), 0)::INTEGER INTO v_user_active_shares
  FROM public.investments AS i
  WHERE i.plan_id = v_investment.plan_id
    AND i.user_id = v_investment.user_id
    AND i.status = 'active';

  SELECT COALESCE(SUM(GREATEST(i.shares_purchased, 0)), 0)::INTEGER INTO v_older_pending_user_shares
  FROM public.investments AS i
  WHERE i.plan_id = v_investment.plan_id
    AND i.user_id = v_investment.user_id
    AND i.status = 'pending'
  ...
  WHERE t.investment_id = p_investment_id AND t.type = 'deposit';
