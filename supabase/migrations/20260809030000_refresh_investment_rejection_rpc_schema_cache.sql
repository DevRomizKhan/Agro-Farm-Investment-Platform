-- Ensure the owner rejection RPCs exist and PostgREST immediately reloads them.
-- This prevents stale schema-cache errors after deployment.
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Pending investment request not found';
  END IF;
END;
$$;
...
BEGIN
  IF public.get_user_role(auth.uid()) <> 'owner' THEN
    RAISE EXCEPTION 'Only owners can reject payments';
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Submitted payment not found';
  END IF;
END;
$$;
